import { Counter, IDL } from "@/config/anchor/idl";
import { programId } from "@/config/anchor/program-config";
...
export default class SolanaRpc {
  ...
  sendTransaction = async (txnBody: any): Promise<Uint8Array | string> => {
    try {
      const solanaWallet = new SolanaWallet(this.provider);
  ...
  getContractSetup = async (): Promise<any[]> => {
    try {
      const solanaWallet = new SolanaWallet(this.provider);
      const connectionConfig = await solanaWallet.request<
        string[],
      ...
      const program = new Program<Counter>(IDL, programId, {
        connection,
      });
